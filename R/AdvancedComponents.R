...
#' user. `initialPosition` sets where it starts (plain `list(top=,
#' left=)`/`list(bottom=, right=)` data) — the window's *live* position
#' while being dragged isn't readable/settable from the server, since that
#' needs a client-side ref. Its parent needs `position: relative` (or
#' similar) for it to be positioned against it.
#'
#' Add `dimensions` (`list(initialWidth=, minWidth=, maxWidth=,
#' initialHeight=, minHeight=, maxHeight=)`) together with a
#' [FloatingWindowResizeHandle()] child to make it resizable (drag or,
#' when the handle has keyboard focus, Arrow keys/Home/End). If `inputId`
#' is set, resizing (drag only — not keyboard) is reported to Shiny:
#' `input[[inputId]]` holds the current `list(width=, height=)` after
#' every resize, and `input[[paste0(inputId, "_resize_start")]]`/
...
#' @param inputId Optional; if set, resize events are reported to Shiny
#'   (see Details). Omit if you don't need to react to resizing
#'   server-side.
#' @export
#' @examples
#' \dontrun{
...
#' )
#' }
FloatingWindow <- function(..., inputId = NULL) {
  mantineElement("FloatingWindow", inputId = inputId, ...)
}
