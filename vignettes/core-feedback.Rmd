...
vignette: >
  %\VignetteIndexEntry{Core: Feedback}
...
An R rewrite of the ["Feedback" category](https://mantine.dev/core/alert/)
on mantine.dev/core. For toast-style popup notifications (as opposed to
the static `Notification()` box below), see `vignette("satellite-packages")`'s
`@mantine/notifications` section instead — that's a different mantine.dev
package (`showMantineNotification()`), not a "core" component.
...
<https://mantine.dev/core/notification/> — a static notification box
for your own layout (e.g. an activity feed item) — distinct from the
toast popups triggered with `showMantineNotification()` in
`vignette("satellite-packages")`, which float over the page and manage
their own stacking/dismissal.
...
composed of `EmptyStateIndicator()`/`EmptyStateTitle()`/
...
- `vignette("satellite-packages")` — toast notifications
  (`@mantine/notifications`) and every other satellite package.
- `vignette("core-overlays")` — `LoadingOverlay()` and other overlay
  components.
