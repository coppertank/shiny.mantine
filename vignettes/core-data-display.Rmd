...
vignette: >
  %\VignetteIndexEntry{Core: Data display}
...
An R rewrite of the ["Data display" category](https://mantine.dev/core/accordion/)
on mantine.dev/core.

## Accordion

<https://mantine.dev/core/accordion/> — a collapsible list of sections;
`AccordionItem()`s (each with `AccordionControl()` + `AccordionPanel()`)
nest inside `Accordion()`. Set `multiple = TRUE` to allow more than one
panel open at once.
...
initials/icon fallback); `AvatarGroup()` overlaps several together.
...
container; `CardSection()` marks a full-bleed section (e.g. an image
touching the card's edges) inside it.
...
(definition-list style), composed of `DataListItem()`/
...
<https://mantine.dev/core/image/> — a styled `<img>` with a `fallbackSrc`
option and Mantine's radius/fit props.
...
<https://mantine.dev/core/table/> — `Table()` and its sub-parts
(`TableThead()`/`TableTbody()`/`TableTr()`/`TableTh()`/`TableTd()`/...)
...
`DataTable()` is not a mantine.dev component — a `shiny.mantine` extra
built on top of `Table()` with search/sort/selection already wired up,
reporting the currently visible/selected row `value`s to Shiny:
...
- `vignette("core-typography")` — `Text()`, `Title()`, `Table()`'s
  typography-adjacent siblings, and more.
- `vignette("extras")` — `SortableList()`/`SortableTable()`, two more
  `shiny.mantine` extras for drag-and-drop reordering.
