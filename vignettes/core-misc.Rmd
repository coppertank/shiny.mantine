...
see `vignette("core-combobox")` for how far the pragmatic `Combobox()`
...
- **`FloatingIndicator`** (and the lower-level `FloatingArrow`) — both
  ...
  own styled components (`SegmentedControl()`, `Tabs()`, ...) already
  provide the equivalent animated-indicator effect out of the box
  without it.
...
- **`Calendar`** (`@mantine/dates`) and its sub-parts (`Month`, `Day`,
  `WeekdaysRow`, ...) — the low-level primitive `DatePicker()`/
  `DatePickerInput()`/`MonthPicker()`/`YearPicker()` are built on;
  customization beyond what those offer goes through a function-based
  `getDayProps` callback, which can't cross the R/JSON bridge.
- **`RichTextEditor`'s `ColorPicker`/`Color`/`UnsetColor` controls** —
  ...
  reduced-scope editor (see `vignette("core-typography")` — no tables,
  images, or collaborative editing either).
